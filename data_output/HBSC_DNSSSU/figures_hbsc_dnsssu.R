...
data_name <- 'HBSC_DNSSSU'
trend_name <- 'MPALC'
file_name <- "20210605_None_[8, 40, 3, 20]_[0.05, 0.78]_[False, 100]_[0.9, 80]_['mov_prev_slope', 'data', None, None, 'max', None]"
...
#sel <- data[data$subgroup %in% c(0,1:25), ]
sel <- data[data$subgroup %in% c(1,3,6,11,16,18,50), ]
pal = c("#fdb462", "#bebada", "#fb8072", "#80b1d3", "#b3de69", "#fccde5", "#636363")
#pal <- c("#636363", gg_color_hue(nr_subgroups+1))
...
data_name <- 'HBSC_DNSSSU'
trend_name <- 'MPALC'
file_name <- "20210605_None_[8, 40, 3, 20]_[0.05, 0.78]_[False, 100]_[0.9, 80]_['mov_prev_slope', 'value', 0.0, False, 'count', 0.01]"
...
#sel <- data[data$subgroup %in% c(0,1:25), ]
sel <- data[data$subgroup %in% c(1,2,13,15,17,18,50), ]
pal = c("#fdb462", "#bebada", "#fb8072", "#80b1d3", "#b3de69", "#fccde5", "#636363")
#pal <- c("#636363", gg_color_hue(nr_subgroups+1))
trend_plot <- ggplot(sel, aes(x = year, y = mov_prev, color = subgroup)) + 
  geom_point(size=0.8) + 
